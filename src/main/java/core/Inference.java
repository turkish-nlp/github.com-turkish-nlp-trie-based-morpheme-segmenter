...
public class Inference {
    ...
    public void run() throws IOException, ClassNotFoundException {
       this.featString = this.generateFeatureString();
    ...
    private String recursiveSplit(Sample sample, String word) {
        ...
        ArrayList<String> possibleSplits = Operations.getPossibleBinarySplits(word, Initialization.getHeuristic());
        ...
        while (newSegments.hasMoreTokens()) {
            ...
            if (frequencyTable.containsKey(morpheme)) {
                if (frequencyTable.get(morpheme) > 0) {
                    ...
                    newLikelihood = newLikelihood + Math.log10(Initialization.getSmoothingCoefficient() / (size));
                    frequencyTable.put(morpheme, 1);
                    size++;
                }
            } else {
